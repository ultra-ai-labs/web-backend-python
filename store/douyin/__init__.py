...
async def batch_update_dy_aweme_comments(aweme_id: str, comments: List[Dict], task_id: str, user_id: str):
    if not comments:
        return
...
async def update_dy_aweme_comment(aweme_id: str, comment_item: Dict, task_id: str, user_id: str):
    ...
    utils.logger.info(
        f"[store.douyin.update_dy_aweme_comment] douyin aweme comment: {comment_id}, content: {save_comment_item.get('content')}, awame_id: {save_comment_item.get('aweme_id')}, user_id: {user_id}")

    await DouyinStoreFactory.create_store().n_store_comment(comment_item=save_comment_item, task_id=task_id, user_id=user_id)
