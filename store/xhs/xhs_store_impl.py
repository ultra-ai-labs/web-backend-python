...
from store.xhs.xhs_store_sql import update_comment_by_comment_id_and_task_id
...
class XhsCsvStoreImplement(AbstractStore):
    ...
    async def n_store_creator(self, creator: Dict, task_id: str):
        await self.n_save_data_to_csv(save_item=creator, store_type="creator", task_id=task_id)
...
class XhsDbStoreImplement(AbstractStore):
    ...
    async def n_store_comment(self, comment_item: Dict, task_id: str, user_id: str):
        """
                ...
                """
        from .xhs_store_sql import (add_new_comment,
                                    n_query_comment_by_comment_id,
                                    update_comment_by_comment_id,
                                    check_user_owns_comment_for_task
                                    )
        comment_id = comment_item.get("comment_id")
        # comment_detail: Dict = await n_query_comment_by_comment_id(comment_id=comment_id, task_id=task_id)
        comment_detail = await check_user_owns_comment_for_task(comment_id=comment_id, user_id=user_id, task_id=task_id)
