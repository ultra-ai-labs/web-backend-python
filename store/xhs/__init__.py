...
async def batch_update_xhs_note_comments(note_id: str, comments: List[Dict], task_id: str, user_id: str):
    if not comments:
        return
    for comment_item in comments:
        await update_xhs_note_comment(note_id, comment_item, task_id, user_id)


async def update_xhs_note_comment(note_id: str, comment_item: Dict, task_id: str, user_id: str):
    ...
    #     "点赞数量": comment_item.get("like_count"),
    #     "用户链接": f"https://www.xiaohongshu.com/user/profile/{user_info.get('user_id')}",
    #     "内容链接": f"https://www.xiaohongshu.com/explore/{note_id}",
    #     "来源标题": "",
    #     "用户昵称": user_info.get("nickname"),
    #     "用户性别": user_info.get("gender"),
    #     "用户年龄": user_info.get("year_old"),
    #     "用户星座": "",
    #     "用户IP": comment_item.get("ip_location"),
    #     "用户地点": "",
    #     "用户ID": "",
    #     "用户签名": "",
    #     "用户关注": "",
    ...
    await XhsStoreFactory.create_store().n_store_comment(local_db_item, task_id, user_id)
