...
# 将ID列表写入Python文件
def write_ids_to_py_file(id_list, output_file='./dy_config.py'):
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("# 指定抖音需要爬虫的笔记ID列表\n")
        f.write("N_DY_SPECIFIED_ID_LIST = [\n")
        for id in id_list:
            f.write(f'"{id}",\n')
        f.write("]\n")
...
# 主函数
def main():
    file_path = '自媒体平台留学帖子收集2.xlsx'  # 更新为实际Excel文件路径
    ...
    driver.quit()  # 关闭浏览器
    write_ids_to_py_file(ids, 'dy_config.py')  # 输出到指定的文件


def get_full_dy_url(short_url):
    try:
...
def get_dy_ids_by_filepath(file_path):
    urls = read_file(file_path)
    ids = []
    for url in urls:
        final_url = get_full_dy_url(url)
        print(final_url)
        id = extract_id_from_url(final_url)
        ids.append(id)
    return ids
...
# 根据excel表格的链接栏目来提取ids



if __name__ == "__main__":
    # main()
    print(get_dy_ids_by_filepath('自媒体平台留学帖子收集.xlsx'))