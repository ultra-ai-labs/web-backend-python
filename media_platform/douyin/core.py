...
class DouYinCrawler(AbstractCrawler):
    ...
    def ninit_config(self, task_id: str, user_id: str, platform: str, login_type: str, crawler_type: str, start_page: int, keyword: str) -> None:
        self.platform = platform
        self.login_type = login_type
        self.crawler_type = crawler_type
        self.start_page = start_page
    ...
    async def nstart(self, id_list):
        ...
        async with async_playwright() as playwright:
            ...
            if self.crawler_type == "search":
                ...
                # Get the information and comments of the specified post
                await self.n_get_specified_awemes(id_list)

            utils.logger.info("[DouYinCrawler.start] Douyin Crawler finished ...")


    async def n_get_specified_awemes(self, id_list):
        """Get the information and comments of the specified post
           It don't get the aweme detail. Updating......
        """
        # semaphore = asyncio.Semaphore(config.MAX_CONCURRENCY_NUM)
        # task_list = [
        #     self.get_aweme_detail(aweme_id=aweme_id, semaphore=semaphore) for aweme_id in id_list
        # ]
        # aweme_details = await asyncio.gather(*task_list)
        # for aweme_detail in aweme_details:
        #     if aweme_detail is not None:
        #         await douyin_store.update_douyin_aweme(aweme_detail)
        await self.batch_get_note_comments(id_list)
    ...
    async def get_specified_awemes(self):
        ...
        semaphore = asyncio.Semaphore(config.MAX_CONCURRENCY_NUM)
        task_list = [
            self.get_aweme_detail(aweme_id=aweme_id, semaphore=semaphore) for aweme_id in config.N_DY_SPECIFIED_ID_LIST
        ]
        aweme_details = await asyncio.gather(*task_list)
        for aweme_detail in aweme_details:
            if aweme_detail is not None:
                await douyin_store.update_douyin_aweme(aweme_detail)
        await self.batch_get_note_comments(config.N_DY_SPECIFIED_ID_LIST)
