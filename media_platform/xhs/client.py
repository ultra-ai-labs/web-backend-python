...
class XiaoHongShuClient(AbstractApiClient):
    ...
    async def get_note_all_comments(self, note_id: str, crawl_interval: float = 1.0,
        ...
        while comments_has_more:
            ...
            if callback:
                await callback(note_id, comments, self.task_id, self.user_id)
    ...
    async def get_comments_all_sub_comments(self, comments: List[Dict], crawl_interval: float = 1.0,
        ...
        for comment in comments:
            ...
            user_short_info = comment["user_info"]
            user_info: Dict = await self.get_creator_info(user_id=user_short_info.get(user_short_info.get("user_id")))
            print(user_short_info)
            if sub_comments and callback:
                await callback(note_id, sub_comments, self.task_id, self.user_id)
