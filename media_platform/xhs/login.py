...
class XiaoHongShuLogin(AbstractLogin):
    ...
    async def login_by_qrcode(self):
        ...
        n_partial_show_qrcode = functools.partial(self.show_qrcode, base64_qrcode_img)
        asyncio.get_running_loop().run_in_executor(executor=None, func=n_partial_show_qrcode)
    ...
    def show_qrcode(self, qr_code:str) -> None:
        ...
        qr.add_data(decode(image)[0].data.decode('utf-8'))
        qr.make(fit=True)
        # Generate ASCII art for the QR code and print it
        qr.print_ascii(invert=True)
        # ascii_img = self.qr_to_ascii(qr)
        # print(ascii_img)
