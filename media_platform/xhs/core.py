...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    def ninit_config(self, task_id: str, platform: str, login_type: str, crawler_type: str, start_page: int, keyword: str, user_id: str) -> None:
        self.platform = platform
        self.login_type = login_type
        self.crawler_type = crawler_type
        self.start_page = start_page
    ...
    async def nstart(self, id_list) -> None:
        playwright_proxy_format, httpx_proxy_format = None, None
        if config.ENABLE_IP_PROXY:
            ip_proxy_pool = await create_ip_pool(config.IP_PROXY_POOL_COUNT, enable_validate_ip=True)
            ip_proxy_info: IpInfoModel = await ip_proxy_pool.get_proxy()
        ...
        async with async_playwright() as playwright:
            ...
            if self.crawler_type == "search":
                ...
                # Get the information and comments of the specified post
                await self.n_get_specified_notes(id_list)
            elif self.crawler_type == "creator":
                # Get creator's information and their notes and comments
                await self.get_creators_and_notes()
            else:
    ...
    async def get_specified_notes(self):
        ...
        semaphore = asyncio.Semaphore(config.MAX_CONCURRENCY_NUM)
        task_list = [
            self.get_note_detail(note_id=note_id, semaphore=semaphore) for note_id in config.N_XHS_SPECIFIED_ID_LIST
        ]
        note_details = await asyncio.gather(*task_list)
        for note_detail in note_details:
            if note_detail is not None:
                await xhs_store.update_xhs_note(note_detail)
        await self.batch_get_note_comments(config.N_XHS_SPECIFIED_ID_LIST)

    async def n_get_specified_notes(self, id_list):
        """Get the information and comments of the specified post"""
        semaphore = asyncio.Semaphore(config.MAX_CONCURRENCY_NUM)
        # task_list = [
        #     self.get_note_detail(note_id=note_id, semaphore=semaphore) for note_id in id_list
        # ]
        # note_details = await asyncio.gather(*task_list)
        # for note_detail in note_details:
        #     if note_detail is not None:
